...
2. The image is a `<figure>` with a `<figcaption>` (semantic HTML, good for SEO); the caption
   doubles as the `alt` text. Thai pages use `captionTh`, English pages use `caption`.
3. Text: Thai page → `textTh` when present (plain text), otherwise the English Markdown body.
4. The About page ships `ProfilePage` JSON-LD with `mainEntity: Person` (shared `personLd()`
